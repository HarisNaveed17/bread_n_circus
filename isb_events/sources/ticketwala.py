...
`paymentProcessingFee` are booking fees, not the ticket). So `price_text` is
always `None` here, and the digest simply leaves the line out. A price fetch
against the event page was built and removed; see CLAUDE.md § Ticketwala
prices for why, before building it again.
"""
