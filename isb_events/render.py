...
def _event_block(event: Event, dates: list[datetime] | None = None) -> str:
    ...
    # A price we don't have is simply left out, like a venue we don't have.
    # A "Check with organiser" placeholder was tried and removed: Ticketwala
    # event pages are blocked from CI (CLAUDE.md § Ticketwala prices do not
    # work from CI), so it landed on every paid event in the digest. A line
